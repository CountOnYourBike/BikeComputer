    ...
    static double getCircValue(Context context, String system, String value){
        ...
        if(system.equals(context.getResources().getString(R.string.ETRTO_system))) {
            ...
            for(Wheel wheel: wheels){
                ...
                if(Double.toString(wheel.mCirc).split("\\.")[0]
                        .equals(value.split(" ")[0]))
                    return wheel.mCirc;
            }
        } else {
            throw new IllegalArgumentException("Wheel system is incorrect: " + system);
        }
        throw new IllegalArgumentException("Unknown wheel size of system " + system + ": " + value);
    }
